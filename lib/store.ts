...
const bucket = (collection: Collection, workspaceId?: string) =>
  workspaceId ? `premiumTemplateSites/${workspaceId}/${collection}` : collection;
const collectionRef = (collection: Collection, workspaceId?: string) =>
  workspaceId
    ? db().collection('premiumTemplateSites').doc(workspaceId).collection(collection)
...
export async function atomic<T>(
    ...
    const task = lock.then(async () => {
      ...
      const unit: Unit = {
        get: async <T>(c: Collection, id: string) =>
          structuredClone(data[bucket(c, workspaceId)]?.[id]) as T | undefined,
        set: (c, id, v) => {
          const target = bucket(c, workspaceId);
          data[target] ??= {};
          data[target][id] = v as RecordData;
        },
        delete: (c, id) => {
          delete data[bucket(c, workspaceId)]?.[id];
        },
      };
  ...
  return db().runTransaction(async (tx) => {
    ...
    const unit: Unit = {
      get: async <T>(c: Collection, id: string) => {
        const s = await tx.get(collectionRef(c, workspaceId).doc(id));
        return s.exists ? (s.data() as T) : undefined;
      },
      set: (c, id, v) => {
        writes.push(() => tx.set(collectionRef(c, workspaceId).doc(id), v));
      },
      delete: (c, id) => {
        writes.push(() => tx.delete(collectionRef(c, workspaceId).doc(id)));
      },
    };
    const result = await fn(unit);
...
export async function get<T>(c: Collection, id: string, workspaceId?: string): Promise<T | undefined> {
  if (localMode()) return (await readLocal())[bucket(c, workspaceId)]?.[id] as T | undefined;
  const s = await collectionRef(c, workspaceId).doc(id).get();
  return s.exists ? (s.data() as T) : undefined;
}
export async function list<T>(c: Collection, uid?: string, workspaceId?: string): Promise<T[]> {
  if (localMode())
    return Object.values((await readLocal())[bucket(c, workspaceId)] ?? {}).filter((v) => !uid || v.uid === uid) as T[];
  let q: FirebaseFirestore.Query = collectionRef(c, workspaceId);

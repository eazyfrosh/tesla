...
Uploads use the existing Firebase Admin service account and Firebase Storage. Enable the Storage bucket for the Firebase project and ensure that service account can read/write objects. The bucket defaults to `NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET`; `FIREBASE_STORAGE_BUCKET` is an optional server override. No new service credentials are needed. Deploy the included rules with `firebase deploy --only firestore:rules,storage --project tesla-337d2` using a Firebase-authorized account. Browser writes remain denied. Vercel deploys application code, not Firebase rules.

Images are limited to 3 MB and PNG/JPEG/WEBP signatures. `uploads` stores metadata; wallet/deposit records store `/api/uploads/{id}` URLs. These URLs stream the Firebase objects through session-protected routes, so proof screenshots are not publicly shared. QR files are administrator-uploaded and visible to signed-in users. The local development fallback stores upload files under ignored `.local-data/uploads`; production always requires Firebase Storage.
